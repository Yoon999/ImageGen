...
- 고정 시드와 무작위 시드 지원
- 생성 중 스트리밍 미리보기
- 목표 장 수를 지정한 순차 생성 및 현재 이미지 완료 후 대기열 중단
- 생성 결과 자동 저장 및 클립보드 복사
- 현재 Anlas 잔액, 최근 생성 비용, Opus V5 생성 잔량 표시
- 프롬프트 태그 자동 완성
...
- 전체 그래프 반복 큐, 큐 사이 0.2초 간격, 현재 이미지 완료 후 현재 실행만 중단
- Generator의 Vibe Transfer 및 Precise Reference 설정을 각 이미지 생성 노드에 적용
- 우하단 생성 프리뷰를 별도 창으로 분리하고 창을 닫아 다시 도킹
